// Buffy Goalkeeper — realistic goal chance
...
// Constants (шире ворота для реалистичного гола)
const GOAL_LEFT = canvas.width * 0.25;
const GOAL_RIGHT = canvas.width * 0.75;
...
// Ball
const ball = { x: canvas.width/2, y: 0, r:28, startY:0, pull:0, vx:0, vy:0 };
...
function resetBall(){
  ball.startY = canvas.height - 80;
  if(window.innerWidth<600) ball.startY = canvas.height - 60;
...
function dragMove(e){
  if(!dragging) return;
  const p = getPos(e);
  const maxPull = ball.startY - 50; // pull почти до верха экрана
  ball.pull = clamp(ball.startY - p.y, 0, maxPull);
...
function shoot(){
  ...
  inShot = true;
  shots++;
  const power = clamp(ball.pull/7 + 6, 6, 50); // максимальная сила
  ball.vy = -power;
  ball.vx = (target.x - ball.x) * 0.018 * (power/12);
  keeper.speed = Math.random() < 0.3 ? 1.2 : 2.5; // случайная скорость
}

// Keeper
function updateKeeper(){
  if(inShot){
    const error = (Math.random() - 0.5) * 60; // ошибка ±30px
    const diff = target.x - (keeper.x + keeper.w/2) + error;
...
function draw(){
  ...
  // draw pull line & indicator
  if(!inShot && dragging){
    const drawY = ball.startY - ball.pull;
    ctx.strokeStyle='rgba(255,255,0,0.9)'; ctx.lineWidth=3;
    ctx.beginPath(); ctx.moveTo(ball.x, drawY); ctx.lineTo(target.x, GOAL_LINE_Y-20); ctx.stroke();
    const pw = clamp(ball.pull/(ball.startY-50),0,1);
